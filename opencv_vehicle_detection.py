...
frames_count, fps, width, height = capture.get(cv2.CAP_PROP_FRAME_COUNT), capture.get(cv2.CAP_PROP_FPS), capture.get(
    cv2.CAP_PROP_FRAME_WIDTH), capture.get(cv2.CAP_PROP_FRAME_HEIGHT)
...
#saving a video file
ret, frame = capture.read()  # import image
...
while True:
    ret, frame = capture.read()  # import image
    if not ret:  # if video finish repeat
